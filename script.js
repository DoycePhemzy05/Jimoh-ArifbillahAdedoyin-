...
const CONFIG = {
  ...
  TIMEZONE_BASE: 'https://api.open-meteo.com/v1/timezone',
 ...
 */
function parseDateInTimeZone(isoDate, timeZone) {
  const [datePart, timePart = '00:00:00'] = isoDate.split('T');
  const [year, month, day] = datePart.split('-').map(Number);
  const [hour, minute, second = '0'] = timePart.split(':').map(Number);
  const candidateUtc = Date.UTC(year, month - 1, day, hour, minute, second);
  ...
  parts.forEach(({ type, value }) => {
    if (type !== 'literal') actual[type] = value;
  });
  ...
  const offset = actualUtc - candidateUtc;
  return new Date(candidateUtc - offset);
}

function formatDate(isoDate, timeZone = undefined) {
  const date = timeZone ? parseDateInTimeZone(isoDate, timeZone) : new Date(isoDate);
 ...
 */
function formatTime(isoDate, timeZone = undefined) {
  const date = timeZone ? parseDateInTimeZone(isoDate, timeZone) : new Date(isoDate);
  return new Intl.DateTimeFormat(undefined, {
    weekday: 'long',
    hour: 'numeric',
...
async function getTimezone(lat, lon) {
  const url = `${CONFIG.TIMEZONE_BASE}?latitude=${lat}&longitude=${lon}`;
  const data = await fetchJson(url);
...
class WeatherApp {
  ...
  async reverseGeocode(lat, lon) {
    ...
    try {
      ...
      const timezone = await getTimezone(lat, lon).catch(() => 'UTC');
      ...
      if (error.message.startsWith('Geolocation service error')) {
        throw error;
      }
      throw new Error('Unable to determine your location with browser location services. Please try searching manually.');
    }
  }
  ...
  exportRecentSearchesCSV() {
    if (this.recentSearches.length === 0 && !this.currentWeather) {
      this.setMessage('No weather data or recent searches to export', 'info');
      return;
    }
    ...
    if (this.currentWeather && this.currentLocation) {
      const current = this.currentWeather.current_weather;
      const weatherDetails = getWeatherDetails(current.weathercode);
      rows.push(['Current Location', `${this.currentLocation.name}, ${this.currentLocation.country}`]);
      rows.push(['Local Time', formatTime(current.time, this.currentTimezone)]);
      rows.push(['Temperature', `${this.convertTemperature(current.temperature)} ${this.getTemperatureUnit()}`]);
      rows.push(['Condition', weatherDetails.label]);
      rows.push(['Wind', `${this.convertWindSpeed(current.windspeed)} ${this.getWindSpeedUnit()} ${getWindDirection(current.winddirection)}`]);
      rows.push([]);
      rows.push(['3-day forecast']);
      rows.push(['Date', 'Condition', 'Min Temp', 'Max Temp']);

      const daily = this.currentWeather.daily;
      if (daily && daily.time && daily.time.length) {
        daily.time.slice(0, 3).forEach((date, index) => {
          const forecastDetails = getWeatherDetails(daily.weathercode[index]);
          const minTemp = `${this.convertTemperature(daily.temperature_2m_min[index])} ${this.getTemperatureUnit()}`;
          const maxTemp = `${this.convertTemperature(daily.temperature_2m_max[index])} ${this.getTemperatureUnit()}`;
          rows.push([formatDate(date, this.currentTimezone), forecastDetails.label, minTemp, maxTemp]);
        });
      }
    ...
    if (this.recentSearches.length > 0) {
      rows.push(['Recent Searches']);
      rows.push(['City Name', 'Search Index']);
      this.recentSearches.forEach((city, index) => rows.push([city, index + 1]));
    }
    ...
    this.downloadCSV(csvContent, 'weather-export.csv');
    this.setMessage('Weather data exported successfully!', 'success');
  }
  ...
  updateWeather(location, weather) {
    ...
    this.currentTimezone = weather.timezone || location.timezone || 'UTC';

    const current = weather.current_weather;
    const weatherDetails = getWeatherDetails(current.weathercode);
    const localTime = formatTime(current.time, this.currentTimezone);
  ...
  updateForecast(daily) {
    ...
    days.forEach((date, index) => {
      ...
      dayEl.innerHTML = `
        <strong>${formatDate(date, this.currentTimezone)}</strong>
